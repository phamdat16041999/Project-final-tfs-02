    ...
    <div class="container-xl">
      <div class="row">
        ...
        <form v-on:submit.prevent="CreateHotel" method="post" class="row">
          ...
          <div
            class="col-12"
            v-for="(index, value) in formData.room"
            :key="value"
          >
            <hr />
            <div class="row">
              ...
              <div class="col-xl-6 col-12">
                <div class="row">
                  <div
                    class="col-12"
                    v-for="options in index.price"
                    :key="options.id"
                  >
                    <label class="form-label"
                      >Price for {{ options.NameOption }}</label
                    >
                    <input
                      type="number"
                      class="form-control"
...
export default {
  ...
  methods: {
    ...
    async CreateHotel() {
      const createHotel = await axios.post("/createhotel", this.formData);
      if (createHotel.status == 200) {
        // chuyen huong sang trang login
        this.err = createHotel.data.Messenger;
        console.log("add success");
        this.$router.push("/Hotelier");
      } else {
        this.err = createHotel.data;
      }
    },
  },
  created() {
    axios.get("http://localhost:8080/option", {}).then((res) =>
